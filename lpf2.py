...
CONNECT_RETRY_MS = const(5000)  # cooldown before retrying connect when hub not present

# ESP32/LMS-ESP32-v2: pini UART pentru Hub LEGO (conform diagrama V2)
# Rx=8 (de la hub), Tx=7 (catre hub)
ESP32_RX_PIN = 8
ESP32_TX_PIN = 7
...
class LPF2(object):
    def __init__(
        ...
        if "RT1060" in implementation[2]:
        ...
        elif "RP2040" in implementation[2] or "Pico" in implementation[2]:
            self.BOARD = PICO
            try:
                from lms_pico import RX_PIN, TX_PIN
            except ImportError:
                RX_PIN = 5   # GP5
                TX_PIN = 4   # GP4
            if tx == None:
            ...
            if uart_n == None:
                self.UART_N = 1  # UART1 on Pico
            print("Raspberry Pi Pico defaults loaded, rx=GP{}, tx=GP{}".format(self.RX_PIN_N, self.TX_PIN_N))
            ...
            self.BOARD = ESP32
            self.RX_PIN_N = rx if rx is not None else ESP32_RX_PIN
            self.TX_PIN_N = tx if tx is not None else ESP32_TX_PIN
            if uart_n == None:
                self.UART_N = 2
            print(
                "LMS-ESP32 loaded, rx=GP{}, tx=GP{}".format(self.RX_PIN_N, self.TX_PIN_N)
            )
    ...
    def heartbeat(self):
        if not self.connected:
            now = utime.ticks_ms()
            if utime.ticks_diff(now, self.last_connect_attempt) < CONNECT_RETRY_MS:
                return
    ...
    def connect(self):
        ...
        try:
            ...
            print("LPF2 connect error:", e)
            self.connected = False

    def _connect_inner(self):
        fast_uart_hub = False
        print("LPF2: init handshake...")
        self.init_pins()
        ...
        for i in range(25):  # Wait for AOK
            ...
            while self.rx_pin.value() == 0:
                ...
                if n > 500:
                    break
                # Wait until rise again (max 500ms to avoid infinite hang)
        ...
        if self.connected:
            ...
            print("Successfully connected to hub with sensor id {}".format(self.sensor_id))
            if not fast_uart_hub:
                self.fast_uart()
        else:
            print("Failed to connect to hub (retry in {}s)".format(CONNECT_RETRY_MS // 1000))
