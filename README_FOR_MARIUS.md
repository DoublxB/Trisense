...
2. Deschizi **consola serială** (Thonny sau `python -m mpremote connect COM?` — înlocuiești COM cu portul tău).
3. În output apare o lină de forma:
...
| **4** | La pornirea ESP în consolă → notezi **IP robot** pentru `PC_VOICE_IP` în `.env` |
...
- `PC_VOICE_IP` greșit în `.env` → TTS/audio TCP către ESPS poate eșua; MQTT poate merge în paralel pentru alte comenzi.
